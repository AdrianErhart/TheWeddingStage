...
export function AuthMenu({
  ...
  const handleLogout = () => {
    setMenuOpen(false);
    window.location.assign(`/logout?t=${Date.now()}`);
  };
    ...
    const loadSession = async () => {
      for (let attempt = 0; attempt < 2; attempt += 1) {
        try {
          ...
          // Keep the last known UI state on transient request errors.
          // This avoids jumping to a logged-out menu while the session is still valid.
        }
      }
    };
          ...
          <>
            ...
            {menuOpen ? (
              <div className="absolute right-0 top-[calc(100%+0.75rem)] w-64 overflow-hidden rounded-3xl border border-black/10 bg-white shadow-xl shadow-black/10">
                ...
                <button
                  type="button"
                  className="block w-full cursor-pointer px-6 py-4 text-left text-base text-black/75 transition hover:bg-black/5"
          ...
          <div className="px-6 py-5">
            ...
            {!isAuthenticated ? (
              ...
              <div className="space-y-1">
                ...
                <button
                  type="button"
                  className="flex min-h-12 w-full cursor-pointer items-center rounded-xl px-3 py-2 text-base text-black/75 transition hover:bg-black/5"
                  onClick={handleLogout}
                >
                  Ausloggen
                </button>
