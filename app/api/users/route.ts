...
export async function POST(request: Request) {
  try {
    ...
    console.info("[register] request received", { email: maskedEmail, role });
    ...
    console.info("[register] user created", {
      email: maskedEmail,
      role,
    ...
    try {
      if (role === "artist") {
        ...
        console.info("[register] artist profile created", {
          email: maskedEmail,
          userId: result.insertedId.toString(),
        });
      }
    } catch (error) {
      console.error("[register] artist profile creation failed", {
        email: maskedEmail,
        userId: result.insertedId.toString(),
    ...
    try {
      ...
      console.info("[register] smtp send starting", {
        email: maskedEmail,
        host: mailConfig.host,
        port: mailConfig.port,
        secure: mailConfig.secure,
      ...
      console.info("[register] smtp send finished", {
        email: maskedEmail,
        userId: result.insertedId.toString(),
      });
    } catch (error) {
      console.error("[register] smtp send failed", {
        email: maskedEmail,
        userId: result.insertedId.toString(),
        error: error instanceof Error ? error.message : String(error),
      });
