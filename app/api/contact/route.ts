...
function getMailerConfig() {
  ...
  if (!fromAddress || !EMAIL_PATTERN.test(fromAddress) || hasNonDeliverableEmailDomain(fromAddress)) {
    return null;
  }

  if (!EMAIL_PATTERN.test(user) || hasNonDeliverableEmailDomain(user)) {
    return null;
  }
...
export async function POST(request: Request) {
  try {
      ...
      return NextResponse.json(
        {
          ok: false,
          message:
            "E-Mail-Versand ist fehlerhaft konfiguriert. Prüfe SMTP_HOST, SMTP_PORT, SMTP_USER und SMTP_FROM (keine .local/.localhost/.invalid/.test Domains).",
        },
        { status: 500 }
      );
    }
    ...
    try {
      ...
      console.error("[contact] confirmation mail failed", {
        from: maskEmail(config.from),
        to: maskEmail(email),
        replyTo: maskEmail(adminEmail),
    ...
    if (message.toLowerCase().includes("domain does not exist") || message.toLowerCase().includes("all recipients were rejected")) {
      return NextResponse.json(
        {
          ok: false,
          message:
            "Mailversand fehlgeschlagen: Mindestens eine E-Mail-Adresse ist auf eine nicht zustellbare Domain gesetzt (z. B. .local). Prüfe CONTACT_ADMIN_EMAIL, SMTP_FROM und die im Formular eingegebene Adresse.",
        },
        { status: 500 }
      );
    }
